...
const threes = pokemon.filter(pokemon => pokemon.id % 3 === 0);
console.log(threes);
...
const highId = pokemon.filter(pokemon => pokemon.id > 99).map(pokemon => pokemon.name);
console.log(highId);

console.log('an array with just the names of the pokémon whose only type is poison');
const poison = pokemon.filter(pokemon => pokemon.types.length == 1 && pokemon.types[0] == 'poison').map(pokemon => pokemon.name);
console.log(poison);
...
const normal = pokemon.filter(pokemon => pokemon.types.includes('normal')).reduce((sum, pokemon) => {
    return sum += 1
}, 0);
console.log(normal);